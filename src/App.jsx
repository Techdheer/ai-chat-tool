...
useEffect(()=>{
  console.log(setselectedHistory);
  askQuestion();
},[selectedHistory]) 
...
if(darkMode=='dark'){
  ...
  return (
   ...
   <select
   onChange={(event)=>setDarkMode(event.target.value)} className="fixed bottom-0  dark:bg-zinc-800 bg-red-100 dark:text-white text-zinc-700 outline-none p-5  "
>
  ...
  <RecentSearch recentHistory={recentHistory} setRecentHistory={setRecentHistory} setselectedHistory={setselectedHistory}/>
    ...
    <div ref={scrollToAns} className="container h-5/6 overflow-y-auto w-full px-4 pt-3 ">
...
{ result && result.map((item, index) => (
  <li key ={index+Math.random()} className='text-left p-0.5'><Answer ans={item} totalResult={result.length} index ={index}/></li>
    ...
    <div className="fixed bottom-5 left-1/2 transform -translate-x-28
             dark:bg-zinc-800 bg-red-100 text-zinc-800 dark:text-white
             w-1/3 p-5 rounded-full border border-zinc-700 
             flex items-center justify-center h-16 shadow-lg ">

    <input type="text" 
     value={question}
     onKeyDown={isEnter}
     onChange={(event)=>setQuestion(event.target.value)}  className="flex-grow bg-transparent outline-none px-3 text-base" placeholder="Ask Anything"
      />
